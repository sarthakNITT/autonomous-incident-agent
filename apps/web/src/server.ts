...
const server = Bun.serve({
    ...
    async fetch(req) {
        ...
        if (req.method === "GET" && (url.pathname === "/" || url.pathname === "/incidents")) {
            try {
                const res = await fetch(`${config.services.state.base_url}/incidents`);
                const incidents = await res.json() as Incident[];
                return new Response(IncidentsPage(incidents), { headers: { "Content-Type": "text/html" } });
            } catch (e) {
        ...
        if (req.method === "GET" && url.pathname.startsWith("/incidents/")) {
            ...
            try {
                const res = await fetch(`${config.services.state.base_url}/incidents/${id}`);
                ...
                if (incident.patch_diff_key) {
                    patch = await storage.downloadText(incident.patch_diff_key).catch(() => null);
                }

                logs = await storage.downloadText(`incidents/${id}/repro/post.txt`).catch(() => null);

                return new Response(IncidentDetailPage(incident, patch, logs), { headers: { "Content-Type": "text/html" } });
            } catch (e) {
