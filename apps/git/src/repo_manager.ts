...
export class RepoManager {
    ...
    async applyPatch(repoPath: string, patchContent: string) {
        ...
        try {
            await this.runGit(["apply", "temp.patch"], repoPath);
        } catch (e) {
            console.error("Apply patch failed, trying 3-way merge or check");
            throw e;
        } finally {
    ...
    private async runGit(args: string[], cwd: string): Promise<void> {
        return new Promise((resolve, reject) => {
            ...
            proc.on("close", (code) => {
                if (code === 0) resolve();
                else reject(new Error(`Git command failed: git ${args.join(" ")} (code ${code})`));
            });
        });
    }
}
